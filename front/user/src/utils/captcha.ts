...
const DEVICE_KEY = 'user_captcha_device_id'

/**
 * 验证码请求工具（裸 axios，不经 request 拦截器：验证码接口返回 anji 的 repCode 而非统一 Result）
 * @Author: YiRanCrazy@gmail.com
 * @Description: 滑块验证码 get/check，并维护设备级 deviceId（后端按「设备+IP」绑定通过标记）
...
/**
 * 获取设备唯一 ID（首次生成后持久化，get/check/login/register 共用同一标识）
 */
